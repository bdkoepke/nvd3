/* nvd3 version 1.8.6-dev (https://github.com/novus/nvd3) 2017-12-08 */
(function(){
...
nv.models.rangeChart = function() {
    ...
    function chart(selection) {
        ...
        selection.each(function(data) {
            ...
            // Setup containers and skeleton of chart
            var wrap = container.selectAll('g.nv-wrap.nv-rangeChart').data([data]);
            var wrapEnter = wrap.enter().append('g').attr('class', 'nvd3 nv-wrap nv-rangeChart nv-chart-' + re.id());
            var gEnter = wrapEnter.append('g');
            var g = wrap.select('g');
            ...
            gEnter.append('g').attr('class', 'nv-x nv-axis');
            gEnter.append('g').attr('class', 'nv-y nv-axis');
            gEnter.append('g').attr('class', 'nv-rangeWrap');
            ...
            wrap.select('.nv-rangeWrap')
                .datum(data.filter(function(d) { return !d.disabled }))
                .call(re);


            wrap.select('.nv-regressionLinesWrap')
                .attr('clip-path', 'url(#nv-edge-clip-' + re.id() + ')');
            ...
            regLine.filter(function(d) {
                ...
                .watchTransition(renderWatch, 'rangePlusLineChart: regline')
                .attr('x1', x.range()[0])
                .attr('x2', x.range()[1])
            ...
            re.dispatch.on('elementMouseout.tooltip', function(evt) {
                tooltip.hidden(true);
                container.select('.nv-chart-' + re.id() + ' .nv-series-' + evt.seriesIndex)
                    .attr('y1', 0);
                container.select('.nv-chart-' + re.id() + ' .nv-series-' + evt.seriesIndex)
                    .attr('x2', distY.size());
            });

            re.dispatch.on('elementMouseover.tooltip', function(evt) {
                container.select('.nv-series-' + evt.seriesIndex)
            ...
            re.dispatch.on('elementMousemove.tooltip', function(evt) {
                tooltip();
            });
    ...
    nv.utils.inheritOptions(chart, re);
    nv.utils.initOptions(chart);
    return chart;
};
